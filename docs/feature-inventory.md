...
| RUN MAP biome identity: exact installed biome artwork; desktop uses a large region pane with lower-anchored name/lean/day/countdown copy and a distinct REGION GUIDE footer, while compact uses a region summary. Both open the guide inside the destination panel. Empty discoveries show NO DISCOVERIES YET. | [x] | [x] |
...
| RUN LEDGER: explicit map-header button on both profiles opens shared stats inside the destination panel, with Back and unchanged calculations. REGION GUIDE uses shared boss/enemy/shop/theme facts, labels regional preferences as non-exclusive and not guaranteed, and retains separate mobile/desktop layouts. Both preserve an open shop/event destination on return. Separate earned future MAP INTEL remains a desktop rail or compact scroll sheet. | [x] | [x] |
