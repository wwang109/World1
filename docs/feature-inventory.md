...
| **SHARED RUN HUD** (`renderRunHud`, `src/game/ui/RunProgressStrip.ts` + `runScreenTemplate.ts`): IDENTICAL header on every run screen (map/prep/event/shop/draft/deck-build in run context) — kicker+title, ONE always-on stats strip `STOP n · DAY n/5 · GOLD g · LV n · LIVES n · BOSSES n` (absolute stop depth, region day 1–5; compact gold/lives/boss labels G/♥/B; LIVES reads danger-red at 1), banked-PL badge in its own slot, and fixed back/secondary/tertiary/primary action slots (map: DECK/BAG, RUN LEDGER, RETIRE, unused primary; other screens retain their own role actions) — every role sits in the SAME rect on every screen regardless of which roles a given screen uses (empty slots stay empty, never reflowed into) | [x] | [x] |
