...
|---|---|---|---|
| **Physical** | vs Armor, scales off **Attack** | scales off Attack | Physical damage only |
| **Magical** | vs Magic Resist, scales off **Magic Power** | scales off Magic Power | Magical damage only |
| **True** | ignores Armor/Magic Resist entirely, scales off the caster's **higher** of Attack/Magic Power | **FLAT** amount — no scaling, no reduction | ALL damage types |

Every card carries exactly one property. It governs which stat the `power`
number scales from and what a typed shield blocks.
...
### Elements (`SkillDef.element`) — required on every Magical card
...
### Weapon types (`SkillDef.weapon`) — required on every Physical *damage* card
...
|---|---|
| `damage` | Deals `power`% of the scaling stat (physical/magical) or a flat `power` (true) as damage. |
| `heal` | Restores `power`% of the scaling stat (physical/magical) or a flat `power` HP (true). |
| `shield` | Grants a typed shield pool worth `power`% of the scaling stat (physical/magical) or a flat `power` (true). |
| `poison` | Ticks `amount` damage on the victim at the start of each global turn for `turns` turns. **Bypasses shields.** |
| `burn` | Ticks `amount` damage on the victim at the start of each global turn for `turns` turns. **Consumed by shields** (unlike poison). |
...
| `cleanse` | Removes the caster's own poisons, burns, stuns, and stat debuffs. No numeric parameters. |
...
|---|---|
| `slowNext` | The enemy's next action becomes `weight` heavier (comes out later). |
| `stagger` | Drains `amount` from the enemy's banked initiative readiness. |
...
| `damage` (true) | `Deal {power}% of your higher power stat as TRUE damage — ignores Armor and Magic Resist.` |
| `heal` (physical) | `Restore {power}% Attack health.` |
| `heal` (magical) | `Restore {power}% Magic Power health.` |
| `heal` (true) | `Restore a flat {power} HP — no scaling, no reductions, always {power}.` |
| `shield` (physical) | `Gain a Physical shield worth {power}% Attack (blocks physical damage only).` |
| `shield` (magical) | `Gain a Magical shield worth {power}% Magic Power (blocks magical damage only).` |
| `shield` (true) | `Gain a {power}-point TRUE shield — blocks ALL damage types.` |
| `poison` | `Poison for {amount} for {turns} turns.` — append `(poison bypasses shields)` the first/only time a card introduces poison, to disambiguate from burn. |
| `burn` | `Burn for {amount} for {turns} turns.` |
| `stun` (turns = 1) | `Stun — the enemy's next performance is consumed.` |
...
| `cleanse` | `Remove your poisons, burns, stuns and debuffs.` |
...
| `negate` | `Negate the next {charges} {magical} attack(s).` — name the `property` in lowercase ("physical"/"magical"), or say "any" for TRUE. Singular "attack" if `charges` = 1. |

### Riders

| Rider | Template |
|---|---|
| `slowNext` | `the enemy's next action is +{weight} weight slower.` (joined to the main clause with `;`) |
| `stagger` | `drain {amount} from the enemy's banked readiness.` (joined with `and`) |
...
3. **Trailing riders** follow the main verb, in the same order they appear in
   ...
   e.g. `poison`, `debuffStat`, `stagger`, `lifesteal`) or `;` (for
   `slowNext` and `comboBonus`, which read as asides rather than parallel
   clauses).
...
- `hamstring` (damage → slowNext): `"Deal 120% Attack physical damage; the
  enemy's next action is +16 weight slower."`
...
- **Stat names**: capitalize proper stat names exactly as in the §1 table
  ...
  effects (the flat number speaks for itself, reinforced by the "no scaling"
  clause on heals).
