> **HISTORICAL** — accurate as of its date; superseded by `docs/combat-model-spec.md` (loop + event log) and `src/game/battleTimeline.ts`. Never cite as current. Describes the deleted one-activation-per-turn loop and legacy BattleScene.
