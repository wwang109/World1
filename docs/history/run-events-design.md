> **HISTORICAL** — accurate as of its date; superseded by `docs/run-structure.md`. Never cite as current. BUILT — the "Status: planned" line below is stale; `src/data/events.ts` + `src/run/events.ts` shipped.
