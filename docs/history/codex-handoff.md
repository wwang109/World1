> **HISTORICAL** — accurate as of its date; superseded by `CLAUDE.md` and `docs/architecture.md`. Never cite as current. The two-agent Codex arrangement has ended; still-true card-template decisions live in `docs/card-template-spec.md`.
...
| 17 | **Day counter is wrong now: a "day" is every NODE VISIT, not a wave.** `RunProgressStrip.ts` renders `DAY ${currentWave} / ${waveCount}` — that was a cosmetic relabel, but the run economy now pays **+1 gold per node committed** (user-locked), so a day is a real, counted thing: 15-20 per run. Please show the actual day (nodes committed so far) over the run's total node count, and keep `WAVE n / 5` as the separate ladder readout it already is. Run state exposes `depth` and `map.depths.length`; ask if you'd rather have an explicit `day`/`dayCount` on RunState and I'll add it. | Engine side landed (Claude, 2026-07-30): `DAILY_INCOME` in `src/run/runState.ts`, awarded in `chooseNode`. | **DONE — Claude, 2026-07-31.** The shared HUD (`renderRunHud`, `src/game/ui/RunProgressStrip.ts`) shows `DAY n` = `run.depth` (node visits), unbounded, no denominator. |
| 18 | **Fight columns now offer TWO foes — render both.** For waves 1-4 the mandatory fight column contains 2 `kind:'fight'` nodes with the same `fightNumber`, distinguished by `RunNode.fightOption: 'standard' \| 'hard'`. The hard one is one title rung and one level above (normal→elite, elite→boss) and pays more gold. Wave 5's boss stays a single node with `fightOption` undefined. The map scene currently assumes a 1-node fight column, so only one option will show. Please render both as pickable choices with a clear risk/reward read (e.g. a STANDARD / HARD badge plus the existing enemy · LV · title preview — `previewEncounter` already resolves each node's own level/title correctly, no engine change needed). | Engine side landed (Claude, 2026-07-30): `fightOption` in `src/run/runMap.ts`, `fightTableEntryForNode` in `runState.ts`. | OPEN |
| 19 | **Show the gold breakdown so daily income is legible.** A fight day now pays daily 1 + fight base 1 + a difficulty win bonus; a non-fight day pays 1; a LOSS still pays that day's 1 (it used to pay nothing). Players can't see where gold comes from. Suggest a small "+1 DAY" tick on arrival at any node and an itemised payout on the battle banner (`+1 DAY · +1 FIGHT · +2 BONUS`) instead of today's single `+N GOLD`. | Numbers: all-win run now nets 27-31 gold (was 14-20); all-loss nets 16-20 (was 0). | OPEN |
| 20 | **The run is now ENDLESS — the map header is wrong.** User-locked 2026-07-30: no victory state, waves keep coming, the map generates lazily. `WAVE n / 5` and `DAY n / 5` both imply a fixed length that no longer exists. Please show `WAVE n` and `DAY n` unbounded (a day = a node visit, see #17), plus **LIVES (3, decrementing)** and **BOSSES CLEARED** — the latter is the run's score. Engine API: `RunState.lives`, `RunState.bossesCleared`, `BOSS_EVERY` in runMap.ts. | Engine landed (Claude, 2026-07-30, e55a9bd). | **DONE — Claude, 2026-07-31.** The shared HUD's stats strip is now `DAY n · WAVE n · GOLD g · LV n · LIVES n · BOSSES n`, unbounded, identical on every run screen (`renderRunHud`/`runScreenTemplate`); LIVES reads danger-red at 1 remaining. |
| 21 | **Add a RETIRE control + a retired/defeat end screen.** `retireRun(state)` (pure, in runState.ts) sets status `'retired'` and is available whenever the run is `'active'` — it's the player's voluntary stop, and the only other ending is running out of lives (`'defeat'`). Status union is now `'drafting' \| 'active' \| 'victory'(legacy, never set) \| 'defeat' \| 'retired'`, so any existing `=== 'victory'` branch is now dead code and should become a retired/defeat summary showing bosses cleared, fights survived, gold, and the final deck. | A run summary screen was already the #3 gap in the design review; this is the natural place for it. | **DONE — Claude, 2026-07-31.** RETIRE is a fixed tertiary HUD slot on every run screen (`renderRetireConfirm` confirm dialog → `retireActiveRun()` in `runStore.ts`); the Run Map's `renderBanner` now handles `'defeat'`/`'retired'` only (bosses cleared/days survived/gold/hero LV/W-L), `'victory'` branch removed. |
| 22 | **A boss loss no longer ends the run** — it costs a life like any other fight. Any UI copy or banner implying "lose to the boss and it's over" needs updating. | | **DONE — Claude, 2026-07-31.** No remaining "boss loss ends the run" copy; the only end banners are `'defeat'` (0 lives) and `'retired'` (voluntary), both showing bosses cleared as the run's score, not a stopping condition. |
| 23 | **Render the new ATTRITION rule in battle.** Engine landed (Claude, 2026-07-30): from turn 15 every living combatant takes escalating **unblockable true damage** — 5, 10, 15… (`(turn - 14) * 5`), shields do NOT absorb it — so long fights now resolve instead of grinding to the 200-turn draw. Event log: a one-off banner `{ turn, kind: 'attritionStart', amount }` (parallel to `suddenDeathStart`/`fatigueStart`, plus the amount), then ordinary `damage` events with the new `source: 'attrition'` (`property: 'true'`, `blocked: 0`, no `sourceCard`/`calculation`) — one per living unit per turn in canonical order, followed by the usual `died` events. Suggest a log banner ("ATTRITION — 5 TRUE to all") plus the existing per-unit damage floaters; the DoT tag styling is the closest existing family. Knobs: `ATTRITION_START_TURN`/`ATTRITION_STEP` in `src/engine/combat/simulate.ts`, per-fight override `CombatConfig.attritionTurn`. | Stalemate test: a mirrored sustain board went from a `draw` at turn 200 to a `win` at turn 27. | OPEN |
