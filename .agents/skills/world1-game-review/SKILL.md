...
- UI/layout: targeted tests, fresh captures for both profiles, relevant browser
  audit, and the full test gate before completion (QUIET tree only; see
  `world1-handoff` §2 — on a live tree run focused files and say so)
- broad or cross-layer work: `npm test` (QUIET tree only), `npm run typecheck`, and
  `npm run build`
