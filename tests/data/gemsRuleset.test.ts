...
describe('data: gem ruleset contract', () => {
  ...
  // THE OPENER HALF OF R1.1 IS STRUCTURAL NOW (2026-09-06). Three tests used
  // to live here, each poisoning a gem's authored `text` so the validator
  // would reject a Core that did not open "This card:", a Charm that did not
  // open "Hero:", and a Sliver borrowing either. `GemDef.text` NO LONGER
  // EXISTS — a gem's face is generated by `renderGemText`, which takes the
  // opener from the payload's OWN category, so the disagreement those tests
  // described is not expressible and the fixture they poisoned cannot be
  // built. The positive form replaces them, asserted on the generator over
  // every shipped gem in `tests/engine/gemTextRegistry.test.ts`: every Core's
  // generated face opens "This card:" and every Charm's opens "Hero:". The
  // NAME half of R1.1 is unchanged and still tested above.
  it('rejects a `text` field where one used to be required', () => {
    const d = clone();
    defOf(d, 'venom_sliver').text = 'Apply Poison 2.';
