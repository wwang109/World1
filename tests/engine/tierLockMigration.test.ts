...
describe('Q1 migration: the one-definition ladder reproduces the deleted restatements EXACTLY', () => {
  ...
  it('...and the SKILL THE LOOP CASTS is identical too, normalizer and all', () => {
    ...
    // `orderCastSinks` over the assembled kit (the rule that a `lifesteal` must
    // trail every hit of its cast). `leeching_fang` is the one card in the book
    // that gets reordered there, and it is in this migration set — so the
    // comparison has to be made at the layer the combat loop actually reads.
    const diffs: string[] = [];
    for (const id of MIGRATED) {
      for (const tier of TIER_ORDER) {
