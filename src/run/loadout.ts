...
import { mergeTierProgress, pointsOf } from './shop';
...
// instance of the same skill prompts a fuse via `mergeTierProgress` (shop.ts).
...
export function canStackMerge(target: StackMergeCard, dragged: StackMergeCard): boolean {
  ...
  if (target.skillId !== dragged.skillId) return false;
  return target.tier !== 'diamond';
}
...
export function stackMergePieces<T extends StackMergeCard>(target: T, dragged: StackMergeCard): StackMergeResult<T> | null {
  ...
  const result = mergeTierProgress(
    { tier: target.tier, points: pointsOf(target) },
    { tier: dragged.tier, points: pointsOf(dragged) },
  );
  return { merged: { ...target, tier: result.tier, points: result.points }, displacedGem: dragged.gem ?? null };
}
