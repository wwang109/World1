...
| `npm run audio:encode` | Encode `audio-src/` SFX masters into the served `public/game-audio/*.ogg` (commit both) |
...
scripts/      fight.ts, balance.ts, run-hud-audit.ts, check-boundaries.mjs,
              ...
              encode-audio.ts (audio masters -> served Ogg)
public/       SERVED VERBATIM by `vite build` — WebP + small badge PNGs only,
              plus public/game-audio/*.ogg (npm run audio:encode output).
art-src/      NOT served. PNG art masters; `npm run art:encode` reads them.
audio-src/    NOT served. Per-SfxKey audio masters + CREDITS.md; `npm run audio:encode` reads them.
```
...
The one non-WebP exception is `public/game-audio/*.ogg`, encoded by
`npm run audio:encode` from `audio-src/` — masters and outputs both
committed; credits in `audio-src/CREDITS.md`. See `docs/audio-design.md`.
